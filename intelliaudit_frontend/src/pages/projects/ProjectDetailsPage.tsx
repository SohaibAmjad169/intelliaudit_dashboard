...
console.log('[ProjectDetailsPage] Energy imports successful');
...
console.log('[ProjectDetailsPage] Water imports successful');

// Debug message to confirm module loading
console.log('[ProjectDetailsPage] All imports successful');
...
import { EnergyNavigationTabs } from '../../components/ui/EnergyNavigationTabs';
...
function ProjectDetailsContent() {
  ...
  const [currentView, setCurrentView] = useState<string>(() => {
    // Initialize from localStorage or default to 'energy'
    const savedView = localStorage.getItem(`project_${projectId}_currentView`);
    return savedView || 'energy';
  });
  ...
  useEffect(() => {
    async function fetchMonthlySummary() {
      ...
      try {
        ...
        console.error("Error fetching monthly summary data for sidebar:", error);
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="space-y-1">
                ...
                {project.property_gross_floor_area && (
                  <div className="flex justify-between">
                    ...
                    <span>
                      {Number(project.property_gross_floor_area).toLocaleString()} ft²
                    </span>
                  </div>
                )}
              ...
              <div className="space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>{Number(utilityData.electric).toLocaleString()} kWh</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="space-y-1">
                ...
                {project.site_total_energy && (
                  <div className="flex justify-between">
                    ...
                    <span>{Number(project.site_total_energy).toLocaleString()} kWh</span>
                  </div>
                )}
                {project.source_total_energy && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Source Energy</span>
                    <span>{Number(project.source_total_energy).toLocaleString()} kWh</span>
                  </div>
                )}
                {project.site_intensity && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Site EUI</span>
                    <span>{Number(project.site_intensity).toLocaleString()} kBtu/ft²</span>
                  </div>
                )}
                {project.source_total_energy && project.property_gross_floor_area && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Source EUI</span>
                    <span>{Number(project.source_total_energy / project.property_gross_floor_area).toLocaleString()} kBtu/ft²</span>
                  </div>
                )}
                {project.direct_ghg_emissions && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">GHG Emissions</span>
                    <span>{Number(project.direct_ghg_emissions).toLocaleString()} kgCO2e</span>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-lg overflow-hidden border border-emerald-500/20">
              <div className="p-3 bg-card/50">
                <EnergyNavigation
                  ...
                  onNavigate={(path) => {
                    setCurrentView(path);
                    localStorage.setItem(`project_${projectId}_currentView`, path);
                  }}
                />
  ...
  }, [project, location.pathname, showProjectId, toggleProjectId, currentView, projectId, navigate, utilityData, calculatedTotalCost]);
  ...
  const renderView = () => {
    ...
    try {
      switch (currentView) {
        ...
        case 'energy':
          return <Equipment projectId={project.id} project={project} />;
        case 'water':
          return <WaterOverview projectId={project.id} projectDetails={project} setProjectDetails={setProject} />;
        case 'photos-overview':
          return <PhotosOverview projectId={project.id} />;
        case 'ecms':
          return <ECMs projectId={project.id} />;
        case 'energy-overview':
          return <EnergyOverview projectId={project.id} project={project} />;
        case 'water-overview':
          return <WaterOverview projectId={project.id} projectDetails={project} setProjectDetails={setProject} />;
        case 'photo-metadata':
          return <PhotoMetadataComponent projectId={project.id} />;
        case 'summary':
      ...
      return (
        <div className="flex flex-col items-center justify-center p-8">
          <h2 className="text-xl font-semibold mb-4 text-red-500">Error Loading View</h2>
  ...
  return (
    <div className="flex-1 h-full">
      {isLoading ? (
        ...
        <>
        {/* Add the tabbed navigation here */}
        <div className="sticky top-0 z-40 bg-background/95 backdrop-blur-sm border-b border-emerald-500/10">
          <div className="px-6 py-2">
            <EnergyNavigationTabs 
              projectId={project.id}
              publicView={false}
              onTabChange={(tabId) => {
                setCurrentView(tabId);
                localStorage.setItem(`project_${projectId}_currentView`, tabId);
              }}
        ...
        {/* Keep the existing download button and content */}
